...
type model struct {
	dialogBoxWrapper *components.DialogBoxWrapper
	logger           slog.Logger
	width            int
	height           int
	isActive         bool
...
func (m *model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	var newDialogBox tea.Model
	newDialogBox, cmd = m.dialogBoxWrapper.Update(msg)
	m.dialogBoxWrapper = newDialogBox.(*components.DialogBoxWrapper)

	return m, cmd
}

func (m *model) View() string {
	title := titleStyle.Render("Dialog Box Enclosing Tabs with List and Viewport")
	componentView := m.dialogBoxWrapper.View()
...
func main() {
	logFile, err := os.OpenFile("dialog_box_testing.log", os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0666)
	if err != nil {
...
The Cloak of Shadows has changed hands countless times throughout history, often through theft or assassination. Its current owner, if it has one, is unknown – which is exactly how the wearer would prefer it. Those who seek the Cloak must be prepared not only to find it but to resist the temptation of the absolute secrecy and power it offers.`),
	...
	// Create the program with the test component
	p := tea.NewProgram(initialModel(tabsComponent, logger), tea.WithAltScreen())
...
func initialModel(component components.Component, logger *slog.Logger) *model {
	dialogBox := components.NewDialogBox(20, 10, "Do you choose:", "Yay", "Nay", logger, component, false)
	return &model{logger: *logger, dialogBoxWrapper: dialogBox, isActive: false}
}
