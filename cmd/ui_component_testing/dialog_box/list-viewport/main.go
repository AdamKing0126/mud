...
func (m *model) View() string {
	baseView := m.dialogBoxWrapper.View()
...
func statusBarView(width int) string {
	return lipgloss.NewStyle().
		...
		Render(fmt.Sprintf("%-*s", width, "enter: select • q: quit • ↑/k up • ↓/j down ←/h, →/l: switch focus"))
}

func (m *model) SetSize(width, height int) {
	m.width = width
	m.height = height
	m.dialogBoxWrapper.SetSize(width, height)
}
...
For those who manage to befriend Zephyr, they can be an invaluable ally, offering unique perspectives and abilities. However, their fluid nature and uncertain loyalties mean that trusting Zephyr completely is always a risk.`),
	...
	p := tea.NewProgram(initialModel(listViewportComponent, logger), tea.WithAltScreen())
...
func initialModel(component components.Component, logger *slog.Logger) *model {
	...
	return &model{dialogBoxWrapper: dialogBox, isActive: false}
}
